...
pub fn install_mod_from_archive(
    ...
    let _tx_step_id = tx::add_step(
        &tx_sql,
        &operation_id,
        1,
        "stage_files",
        r#"{"archive":"selected"}"#,
        Some(r#"{"compensation":"delete_staging_dir"}"#),
    )?;
...
pub fn recalc_conflicts(conn: &Connection, profile_id: &str) -> Result<usize, CoreError> {
    ...
    let mut stmt = conn.prepare(
        "SELECT id, name, priority
         FROM mods
         WHERE enabled = 1
         ORDER BY priority DESC, installed_at DESC",
    )?;
    ...
    for row in rows {
        let (mod_id, mod_name, priority) = row?;
        let slug = mod_name.to_lowercase().replace(' ', "-");
        let key = format!("Data/{}.esp", slug.split('-').next().unwrap_or("mod"));
