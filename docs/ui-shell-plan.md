...
## Splash (старт приложения)

- При запуске показывается отдельное компактное окно с логотипом Pantheon и индикатором загрузки; после готовности ядра (БД, плагин) окно закрывается и открывается основной shell.
- При ошибке старта splash не должен зависать бесконечно — таймаут или краткое сообщение (реализация в `apps/desktop` + `src-tauri`).
- См. также [`pantheon-implementation-status.md`](pantheon-implementation-status.md).
